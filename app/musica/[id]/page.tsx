...
import { calcularIntervaloSemitons, transporCifraCompleta, NomeNota } from '@/lib/music-theory';
...
export default function MusicaPage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ehFavoritoNuvem(song.id).then(setFavorito);
    } else {
  ...
  const semitons = tomDetectado ? calcularIntervaloSemitons(song.tomOriginal as NomeNota, tomDetectado) : 0;
  const cifraExibida = tomDetectado ? transporCifraCompleta(song.cifra, semitons) : song.cifra;
  ...
  async function toggleFavorito() {
    if (user) {
      if (favorito) {
        await removerFavoritoNuvem(song!.id);
        setFavorito(false);
      } else {
        await adicionarFavoritoNuvem(song!.id, song!.titulo, song!.artista);
        setFavorito(true);
      }
    } else {
      const novaLista = alternarFavorito(song!.id, song!.titulo, song!.artista);
  ...
  return (
    <main>
      <div className="no-print mb-4 flex items-center justify-between">
        <button onClick={() => router.push('/')} className="text-sm text-text-dim">← Voltar</button>
        <div className="flex items-center gap-2">
          <button onClick={toggleFavorito} className="flex h-9 w-9 items-center justify-center rounded-full border border-border bg-panel" title="Favoritar">
            {favorito ? '❤️' : '🤍'}
          </button>
          <ControleFonte tamanho={tamanhoFonte} onAumentar={() => setTamanhoFonte((t) => Math.min(24, t + 1))} onDiminuir={() => setTamanhoFonte((t) => Math.max(12, t - 1))} />
          <button onClick={() => setAfinadorAberto(true)} className="flex h-9 w-9 items-center justify-center rounded-full border border-border bg-panel" title="Afinador">🎸</button>
          <button onClick={() => window.print()} className="flex h-9 w-9 items-center justify-center rounded-full border border-border bg-panel" title="Imprimir">🖨️</button>
        </div>
      </div>
      ...
      <div className="lg:flex lg:gap-8">
        <Sidebar>
          <div className="no-print">
            <header className="mb-5">
              <h1 className="font-display text-2xl font-bold md:text-3xl">{song.titulo}</h1>
              <p className="text-sm text-text-dim md:text-base">{song.artista}</p>
            </header>
            <div className="mb-4 flex flex-wrap items-center gap-2 text-sm">
              <span className="rounded-full border border-border bg-panel px-3 py-1">Tom original: {song.tomOriginal}</span>
              {tomDetectado && (
                <span className="rounded-full bg-violeta/15 px-3 py-1 font-medium text-violeta">
                  Seu tom: {tomDetectado} ({semitons > 0 ? '+' : ''}{semitons} semitons)
                </span>
              )}
            </div>
            {!tomDetectado ? (
              ...
              <div className="mb-4 flex items-center justify-between rounded-xl border border-border bg-panel p-3">
                ...
                <button onClick={resetarTom} className="text-sm font-medium text-violeta">Cantar de novo</button>
              </div>
            )}
          </div>
        </Sidebar>
        ...
        <div className="min-w-0 flex-1">
          <div className="hidden print:block print:mb-4">
            ...
            <p className="text-sm">{song.artista} — Tom: {tomDetectado || song.tomOriginal}</p>
          </div>
